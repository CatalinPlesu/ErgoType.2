...
from kle_model import *

# --- Configuration ---
# Define the default directory for keyboard layouts
DEFAULT_KEYBOARD_DIR = os.path.join(os.path.dirname(__file__), "..", "kle_keyboards")
# --- End Configuration ---
...
class KeyboardAnnotatorDPG:
        ...
        if app_data and 'file_path_name' in app_data:
            ...
            try:
                ...
                if not self.current_file_path:
                    raise ValueError("Original file path unknown. Please load from a file first.")
                                    ...
                                    if key_counter[0] < len(self.keyboard.keys):
                                        ...
                                        if has_annotations:
                                            if new_row and isinstance(new_row[-1], dict):
                                                ...
                                                updated_props.update(prop_dict_to_add)
                                                new_row[-1] = updated_props
                                            else:
                                                # Prepend the new property dict
                                                new_row.append(prop_dict_to_add.copy()) # Append a copy
    ...
    def toggle_sticky_finger(self, sender, app_data, user_data):
        ...
        dpg.set_value("sticky_finger_btn", "LOCKED FINGER" if self.sticky_finger else "FINGER")
    ...
    def toggle_sticky_hand(self, sender, app_data, user_data):
        ...
        dpg.set_value("sticky_hand_btn", "LOCKED HAND" if self.sticky_hand else "HAND")
        self.update_status_sticky()
    ...
    def toggle_sticky_homing(self, sender, app_data, user_data):
        ...
        dpg.set_value("sticky_homing_btn", "LOCKED HOMING" if self.sticky_homing else "HOMING")
        self.update_status_sticky()
    ...
    def update_status_sticky(self):
        ...
        if sticky_modes:
            ...
            dpg.set_value("status_text", "Ready - Click keys to assign attributes")
    ...
    def render_keyboard_preview(self):
        ...
        with dpg.group(parent="keyboard_preview_group"):
            try:
                ...
                preview_width = 900
                preview_height = 500
                scale_x = preview_width / (max_x - self.min_x) if (max_x - self.min_x) > 0 else 1
                scale_y = preview_height / (max_y - self.min_y) if (max_y - self.min_y) > 0 else 1
                self.scale = min(scale_x, scale_y, 30)
                
                # Create draw list
                self.keyboard_drawlist = dpg.add_drawlist(width=preview_width, height=preview_height)
                ...
                for i, key in enumerate(self.keyboard.keys):
                    try:
                        ...
                        text_x = max(x + 2, min(text_x, x + width - len(label) * 4))
                ...
                with dpg.item_handler_registry(tag=f"mouse_handler_reg_{id(self)}"):
                    dpg.add_item_clicked_handler(callback=self.on_keyboard_click)
                dpg.bind_item_handler_registry(self.keyboard_drawlist, f"mouse_handler_reg_{id(self)}")
                    
            except Exception as e:
                print(f"Error creating keyboard preview: {e}")
                dpg.add_text("Error loading keyboard preview", color=[255, 0, 0])
    ...
    def on_keyboard_click(self, sender, app_data):
        ...
        try:
            ...
            absolute_drawlist_x = primary_window_pos[0] + preview_group_pos[0] + drawlist_pos[0]
            absolute_drawlist_y = primary_window_pos[1] + preview_group_pos[1] + drawlist_pos[1]
        ...
        for i, key in enumerate(self.keyboard.keys):
            try:
                ...
                if dpg.does_item_exist(rect_tag):
                    dpg.configure_item(rect_tag, fill=bg_color, color=border_color)
    ...
    def run(self):
        ...
        load_dir = DEFAULT_KEYBOARD_DIR if os.path.isdir(DEFAULT_KEYBOARD_DIR) else "."
        save_dir = DEFAULT_KEYBOARD_DIR if os.path.isdir(DEFAULT_KEYBOARD_DIR) else "."
        ...
        with dpg.file_dialog(directory_selector=False, show=False, callback=self.load_keyboard_callback, tag="load_file_dialog", width=700, height=400, default_path=load_dir):
            dpg.add_file_extension(".json", color=(255, 255, 0, 255)) # Yellow for .json
        ...
        with dpg.window(tag="Primary Window", width=1400, height=900):
            ...
            with dpg.group(horizontal=True):
                dpg.add_button(label="📁 Load Layout", callback=self.show_load_dialog, width=120, height=30)
                dpg.add_button(label="💾 Save As", callback=self.show_save_dialog, width=120, height=30)
                dpg.add_separator()

            # Status Bar
            with dpg.group(horizontal=True):
                dpg.add_text("Status:", tag="status_label", color=[200, 200, 200])
                dpg.add_text("Ready - Load a keyboard layout to begin", tag="status_text", color=[255, 255, 255])

            # File Info
            dpg.add_text("No file loaded", tag="file_path_text", color=[150, 150, 150])
            dpg.add_separator()
            ...
            with dpg.group(horizontal=True):
                ...
                with dpg.child_window(width=480, height=800):
                    dpg.add_text("🎨 STICKY PAINT CONTROLS", color=[255, 255, 100])
                    dpg.add_separator()
                    
                    # Sticky controls section
                    with dpg.group():
                        dpg.add_text("Finger Assignment:", color=[255, 255, 255])
                        with dpg.group(horizontal=True):
                        ...
                        with dpg.group(horizontal=True):
                            dpg.add_checkbox(label="Enable", tag="paint_homing_checkbox")
                    ...
                    with dpg.group(tag="inspector_group", show=False):
                        ...
                        dpg.add_text("Click a key to edit", tag="key_info_text", color=[255, 255, 255])
                        dpg.add_spacer(height=5)

                        dpg.add_text("Finger:", color=[255, 255, 255])
                        dpg.add_combo(items=[f.name for f in Finger], tag="finger_combo", width=200)
                        dpg.add_spacer(height=5)

                        dpg.add_text("Hand:", color=[255, 255, 255])
                        dpg.add_combo(items=[h.name for h in Hand], tag="hand_combo", width=200)
                        dpg.add_spacer(height=5)

                        dpg.add_checkbox(label="Homing Key", tag="homing_checkbox")
                        dpg.add_spacer(height=10)

                        with dpg.group(horizontal=True):
                            apply_button = dpg.add_button(label="Apply Changes", callback=self.apply_changes_callback, width=120)
                            clear_button = dpg.add_button(label="Clear", callback=self.clear_key_callback, width=80)
                    
                    # Color legend
                    dpg.add_separator()
                    with dpg.group():
                        dpg.add_text("🎨 COLOR LEGEND", color=[200, 150, 255])
                        dpg.add_text("Background Colors = Fingers:", color=[255, 255, 255])
                        dpg.add_text("● Light Orange: Thumb", color=[255, 200, 150])
                        dpg.add_text("● Light Green: Index Finger", color=[200, 255, 200])
                        dpg.add_text("● Light Blue: Middle Finger", color=[200, 240, 255])
                        dpg.add_text("● Light Purple: Ring Finger", color=[240, 200, 255])
                        dpg.add_text("● Light Pink: Pinky", color=[255, 200, 240])
                        dpg.add_text("● Light Gray: Unassigned", color=[220, 220, 220])
                        
                        dpg.add_spacer(height=5)
                        dpg.add_text("Border Colors = Hands:", color=[255, 255, 255])
                        dpg.add_text("● Blue Border: Left Hand", color=[0, 100, 255])
                        dpg.add_text("● Red Border: Right Hand", color=[255, 50, 100])
                        dpg.add_text("● Purple Border: Both Hands", color=[150, 50, 200])
                        dpg.add_text("● Gray Border: Unassigned", color=[100, 100, 100])
                        
                        dpg.add_spacer(height=5)
                        dpg.add_text("● White Circle: Homing Key", color=[255, 255, 255])
                ...
                with dpg.child_window(width=900, height=800):
                    ...
                    with dpg.child_window(tag="keyboard_preview_group", width=880, height=750,
                                        border=True):
                        dpg.add_text("Load a keyboard layout to see preview", color=[100, 100, 100])

        # Set the primary window
        dpg.create_viewport(title='Enhanced Keyboard Layout Annotator', width=1400, height=900)
        dpg.setup_dearpygui()
        dpg.show_viewport()
