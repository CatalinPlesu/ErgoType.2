...
class KeyboardPhenotype:
    ...
    def fitness(self, markov_chain, simulated_presses=1_000_000, depth=1, max_total_branches=5_000_000):
        """
        Fitness function with smart branch distribution:
        - Levels 1 & 2: Fully saturated (explore all branches)  
        - Level 3+: Evenly distribute remaining branch budget

        Args:
            markov_chain: The markov chain data
            simulated_presses: Target number of key presses to simulate
            depth: Maximum depth to explore  
            max_total_branches: Total branch budget (e.g., 1M)
        """
        ...
        # Phase 1: Count branches at levels 1 & 2 (fully saturated)
        branches_level_1_2 = 0

        def count_saturated_branches(current_key, current_depth, path_keys=[]):
            nonlocal branches_level_1_2

            if current_depth <= 2:
                branches_level_1_2 += 1

            # Stop counting after level 2
            if current_depth >= 2:
                return
            ...
            if (current_key in markov_chain['markov_chain'] and
                ...
                for next_key in next_keys.keys():
                    count_saturated_branches(
                        next_key, current_depth + 1, current_path)

        # Count branches for levels 1 & 2
        for start_key in markov_chain['markov_chain'].keys():
            if start_key in self.char_key_map.keys():
                count_saturated_branches(start_key, 1, [])

        # Calculate remaining budget for level 3+
        remaining_branches = max_total_branches - branches_level_1_2

        if remaining_branches <= 0:
            print(f"""Warning: Levels 1&2 use {
                  branches_level_1_2:,} branches, exceeding budget of {max_total_branches:,}""")
            remaining_branches = 1000  # Fallback minimal budget

        print(f"Level 1&2 branches: {branches_level_1_2:,}")
        print(f"Remaining budget for level 3+: {remaining_branches:,}")

        # Phase 2: Execute with budget control
        branches_used = 0
        level_3_plus_branches = 0

        def dfs_traverse(current_key, current_depth, path_probability=1.0, path_keys=[]):
            nonlocal total_cost, total_presses, paths_explored, branches_used, level_3_plus_branches

            branches_used += 1
            current_path = path_keys + [current_key]

            # For level 3+, check if we're over budget
            if current_depth >= 3:
                level_3_plus_branches += 1
                if level_3_plus_branches > remaining_branches:
                    return  # Skip this branch - over budget

            # If we've reached target depth, simulate the path
            if current_depth == depth:
                ...
                for key_char in current_path:
                    if key_char in self.char_key_map.keys():
            ...
            if (current_key in markov_chain['markov_chain'] and
                ...
                # For level 3+, we need to sample to stay within budget
                if current_depth >= 2:  # Next level will be 3+
                    # Calculate how many branches we can afford
                    remaining_budget = remaining_branches - level_3_plus_branches

                    if remaining_budget < len(next_keys):
                        # Sample proportionally by probability
                        transitions = list(next_keys.items())
                        probabilities = [t[1]['percentage']
                                         for t in transitions]
                        total_prob = sum(probabilities)
                        normalized_probs = [
                            p/total_prob for p in probabilities]

                        # Sample without replacement
                        selected_indices = random.choices(
                            range(len(transitions)),
                            weights=normalized_probs,
                            k=min(remaining_budget, len(transitions))
                        )

                        next_keys = {
                            transitions[i][0]: transitions[i][1] for i in selected_indices}

                for next_key, next_data in next_keys.items():
                    # Stop if we're over budget for level 3+
                    if current_depth >= 2 and level_3_plus_branches >= remaining_branches:
                        break
        ...
        print(f"Total branches used: {branches_used:,}")
        print(f"Level 3+ branches used: {level_3_plus_branches:,}")
        print(f"""Branch budget utilization: {
              branches_used/max_total_branches:.1%}""")
